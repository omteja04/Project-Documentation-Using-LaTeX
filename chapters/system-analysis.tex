...
\subsection{Actors}
\begin{enumerate}
    \item \textbf{End User / Web Surfer:} The primary consumer of the system. They browse the internet and rely on the SafeSurf system to protect them from malicious websites. They interact with the system primarily through the Chrome extension and the web dashboard \cite{ref23}.
    ...
    \item \textbf{SafeSurf API (System Actor):} The FastAPI backend system that processes incoming requests, orchestrates the layered detection pipeline, interfaces with the ML models, and returns unified JSON risk analysis responses \cite{ref20}.
...
\begin{enumerate}
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./images/chrome-extension.png}
...
The detection engine utilizes a hybrid approach, merging deterministic heuristics with probabilistic machine learning models \cite{ref8}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{./images/dns.png}
\caption{DNS Guard}
\label{fig:dns}
\end{figure}
...
\noindent
A robust ensemble algorithm utilized for scikit-learn compatibility and overall generalization \cite{ref18}. It analyzes 111 deep structural features (character counts, DNS TTL, SSL properties, WHOIS timing) derived from comprehensive phishing datasets \cite{ref2}. The Voting Classifier internally aggregates predictions from:
\begin{itemize}
    \item Logistic Regression
    \item Linear Support Vector Classifier (LinearSVC)
    \item Random Forest Classifier
    \item Histogram-based Gradient Boosting Classifier
    \item eXtreme Gradient Boosting (XGBoost) \cite{ref7, ref19}
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{./images/multi-model-voting.png}
...
\noindent
An algorithm focusing solely on the lexical shape and text strings within the URL. It utilizes a CountVectorizer \cite{ref18} to generate token arrays and scores the visual characteristics using a LogisticRegression model.


\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{./images/nlp.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{./images/weighted-soft-voting.png}
