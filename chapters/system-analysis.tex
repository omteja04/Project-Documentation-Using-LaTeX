...
\subsection{Actors}
\begin{enumerate}
    \item \textbf{End User / Web Surfer:} The primary consumer of the system. They browse the internet and rely on the SafeSurf system to protect them from malicious websites. They interact with the system primarily through the Chrome extension and the web dashboard.
...
\noindent
A robust ensemble algorithm utilized for scikit-learn compatibility and overall generalization \cite{ref18}. It analyzes 111 deep structural features (character counts, DNS TTL, SSL properties, WHOIS timing). The Voting Classifier internally aggregates predictions from:
\begin{itemize}
...
\noindent
An algorithm focusing solely on the lexical shape and text strings within the URL. It utilizes a CountVectorizer to generate token arrays and scores the visual characteristics using a LogisticRegression model.
